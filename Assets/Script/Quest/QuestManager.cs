...
public class QuestManager : MonoBehaviour
{
    ...
    public void ProgressTalkObjective(string dialogueKey)
    {
        foreach (var quest in activeQuests.Values)
        {
            foreach (var obj in quest.objectives)
            {
                if (obj is TalkToNPCObjective talkObj &&
                    talkObj.DialogueKey == dialogueKey &&
                    !talkObj.IsCompleted)
                {
                    talkObj.RecordTalk();
        ...
        Debug.LogWarning($"No active TalkToNPC objective found for key: {dialogueKey}");
    }
}